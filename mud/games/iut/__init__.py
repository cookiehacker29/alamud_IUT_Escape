...
from mud.actions import (
    ...
    PushAction, TeleportAction, EnterAction, LeaveAction, PlayAction, ReadAction, TalkAction, UseInAction,
    PayAction, PayWithAction, FillAction, FillWithAction
)
...
def make_rules():
    ...
    return (
        ...
        (InspectAction  , r"(?:r|regarder|inspecter|observer) %s(\S+)" % DETS),
	    ...
	    (PlayAction     , r"jouer %s(\S+)" % DETS),
        (ReadAction     , r"lire %s(\S+)" % DETS),
        (TalkAction     , r"parler %s(\S+)" % DETS),
        (UseInAction    , r"utiliser %s(\S+) (?:avec |)%s(\S+)" % (DETS,DETS)),
        (PayAction      , r"payer %s(\S+)" % DETS),
        (PayWithAction  , r"payer %s(\S+) avec %s(\w+)" % (DETS,DETS)),
        (FillAction     , r"remplir %s(\S+)" % DETS),
        (FillWithAction , r"remplir %s(\S+) avec %s(\w+)" % (DETS,DETS)),
    )
